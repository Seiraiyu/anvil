 ...
 *
 * It runs through `runCcQuery` in read-only plan mode (Read/Grep/Glob allowed, edits blocked, gated by
...
export interface IntakeModelOptions {
  ...
  signal?: AbortSignal;
  cc?: CcSeam; // fake-cc test seam so tests script the reply without spawning the real CLI
}
...
export async function modelIntake(ctx: IntakeModelContext, opts: IntakeModelOptions): Promise<IntakeOverlay> {
  ...
  try {
    const res = await runCcQuery(prompt, {
      model: opts.model,
